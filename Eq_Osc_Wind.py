...
#----------------------------------------------------------------------------------------------------------
    ...
    date = '2019_11_05/pac_MLD_100_sig2_65'
    ...
    k_bottom_w        = 65   # w-level of the bottom (k = 0 is at z = 0; the bottom is at w-level k)  level 65 is at 3995 metres in the 75 level grid
                             ...
                             # 65 is the usual choice ; 70 is a sensible alternative; 75 is not great because some values are missing   

    # Set expt_name to be used for output files
    expt_name  = region_lc + '_' + sig_type + '_' + str(k_bottom_w) + '_Rf_Xt_Yt_Pt_Of_Bt_Ff_Ct_Apr_Eq_strat_ls_ics_20_100_segs'
    ...
    l_force_fix       = False    #        "      forces are fixed in the least squares fit solution (depending on l_wind_x, l_wind_y, l_press) (F)
...
# 0.2 specify the number of vertical modes to calculate and the depth of the wind penetration to use as inputs;
    ...
    n_vertical_modes     = 6   
    MLD                  = 100.      # depth of wind penetration in metres (this is described as the MLD, mixed layer depth, in the documentation)  
...
# note that dz_layers values are all negative 
    if ll_vertical_modes_shooting:
       c_mode, p_mode, dz_layers, H_bottom = vertical_eigenmodes_shooting( k_bottom_w, deps, pot_rho, n_vertical_modes, file_out )
    else:
# vertical_eigenmodes (direct solution) is not working. H_bottom should be returned  
       c_mode, p_mode, dz_layers, H_bottom = vertical_eigenmodes( k_bottom_w, deps, pot_rho, n_vertical_modes, file_out )
...
#  24/07/2017 removed gdef * rho_0 from expressions for v_mode, u_mode and press_w_e_mode
...
#             forced_ts =  (dtauy_dt_ts - f_taux_ts - f_press_w_e_ts ) / omega_sq
          ...
          if l_plot_forces:
             ...
             end_segments = first_segment + max_no_segments*inc_segments 
             Force_plots(plotfilename, input_file_time_step, first_segment, end_segments, dtauy_dt_ts, f_taux_ts, f_press_w_e_ts, ttl, file_out)   
...
#      print("Corr. MOCR v MOC = ",'{0:.2f}'.format(cc1[1,0]**2)," ",'{0:.2f}'.format(cc2[1,0]**2))
       ...
       fname = filestem + '_MOC_m_vert_' + str(m_vert) + '_j_merid_'+ str(j_merid) + '.nc'  
       print(fname)
       ...
       fname = filestem + '_MODES_m_vert_' + str(m_vert) + '_j_merid_'+ str(j_merid) + '.nc'  
       print(fname)
