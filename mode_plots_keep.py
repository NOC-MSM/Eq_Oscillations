...
def vmode_plots(plotfilename, depths, p_mode):
    ...
    for m in range (p_mode.shape[0]) :
       plt.plot(p_mode[m,:],depths,linewidth=2,label=r'$m=$'+str(m+1))
    plt.title("(a) Vertical mode structure", fontsize=16)
...
def mmode_plots(plotfilename, lats_v, merid_mode_set, title=None):
    ...
    for n in range (merid_mode_set.shape[0]) :
       plt.plot(lats_v,merid_mode_set[n,:],linewidth=2,label=r'$n=$'+str(n))
    plt.title("(b) Meridional mode structures for "+title, fontsize=16)
